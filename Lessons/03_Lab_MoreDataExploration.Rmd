...
-   **Vectors** 1 dimensional structure that contains elements of the same type.All numeric, chr,or etc.We can add a dimension and becomes a matrix.
...
# Vectors ----
vector1 <- c(1,2,5.3,6,-2,4) # numeric vector, c is concatenate
vector1
vector2 <- c("one","two","three") # character vector, strings
vector2
vector3 <- c(TRUE,TRUE,TRUE,FALSE,TRUE,FALSE) #logical vector, boolean
vector3
...
list1[[2]] #2 arguments in one line
...
```{r DataExercise-1a}
high_temp_f <- c(64, 71, 72, 63, 51, 52, 61, 64, 67, 73)
low_temp_f <- c(42, 49, 60, 46, 32, 32, 41, 47, 52, 54)
```
...
```{r DataExercise-1b}
#rounded and specify 2
tocelsius1 <- function(F){
  celsius1 <- round(((F-32)*5/9),2)
  return(celsius1)
}
...
high_temp_cel <- tocelsius(high_temp_f)
low_temp_cel <- tocelsius(low_temp_f)
...
```{r DataExercise-1c}
dataframe1 <- data.frame(high_temp_f,low_temp_f,high_temp_cel,low_temp_cel)
colnames(dataframe1)<-c('high_temp_f','low_temp_f','high_temp_cel','low_temp_cel')
```
...
```{r DataExercise-1d}
summary(dataframe1)
summary(dataframe1$high_temp_f)

sd(dataframe1$high_temp_f)
```
