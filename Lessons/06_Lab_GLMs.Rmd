...
#distance to the line of best fit, anything above the dotted red lines would be considered outlier. regression coefficient not affected means the other elements can't be greatly affected either
#don't add the same effect (variables) twice
#means are different overall but we can group the ones that are similar
#Ttest tells u if means are equal or not
...
cor.test(EPAair$Ozone, EPAair$PM2.5)

#install.packages("corrplot")
OzoneandPM2.5 <- 
  EPAair %>%
  select(mean_Lat:Ozone) %>%
  na.omit() #eliminate columns overall so na.omit and drop would be the same thing here
EPAairCorr <- cor(OzoneandPM2.5)
EPAairCorr
...
#Estimate pm2.5 with all the other variables iwth AIC, when you remove year you improve,
#they are adding more information to the model that the matrix is not showing
