...
Temp_BR_processed <-
  ...
  separate(Statistics,c("Null","Month","Null2")," ") %>% #3rd argument is separator
  mutate( Date = my(paste0(Month,"-",Year))) %>% 
...
Days1 <- as.data.frame(seq(as.Date("1901-01-01"), as.Date("2016-12-01"), by = "month"))

colnames(Days1)[1] <- "Date"

temp.rain <- left_join(Days1, BR_complete, by = "Date")
...
ggplot(BR_complete, aes(x = Date, y = Rainfall
  ...
  geom_line() +
  labs(x = "Time", y = expression("Rainfall"))+
  geom_smooth()
...
ggplot(BR_complete, aes(x = Date, y = Temperature_C
  ...
  geom_line() +
  labs(x = "Time", y = expression("Temperature"))+
...
rain_data_ts <- ts(BR_complete$Rainfall,
                   start=c(f_year,f_month),
                   frequency=12) 
rain_data_ts
```
