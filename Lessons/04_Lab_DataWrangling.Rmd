...
# dplyr filtering
NTL.phys.data.surface <- filter(NTL.phys.data, depth == 0) #we don't need quotes because depth is numeric

# Choose multiple conditions to filter
summary(NTL.phys.data$lakename)
NTL.phys.data.PeterPaul <- 
  filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake")) #%in% everytime lakename matches one of the elements
...
NTL.phys.data.1999<- filter(NTL.phys.data, year4==1999) #class of year4 is numeric

# Exercise 2: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.
NTL.phys.data.Tuesday1990to1999 <- filter(NTL.phys.data, lakename == "Tuesday Lake", 
                                            year4 %in% c(1990:1999))

```
Question: Why don't we filter using row numbers?

> Answer: Because we don't know the positions of the rows we want
...
#na.omit doesn't drop the n.a observations, na.action=na.omit
```
...
NTL.phys.data.wider <-
  ...
  pivot_wider(names_from = depth, 
              values_from = temperature_C) #NA values
```
