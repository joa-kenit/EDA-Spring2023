...
ggplot(data=data.frame(x=0,y=0))+
  geom_point(aes(x=x,y=y)) + #defining aes globally
  geom_text(
    x=0,
    y=0.025,
    label=expression(paste("Mean is ", mu,",", R^{2},'=0.6'))
    ) #expression is useful for greek letters, check math jax
...
ggplot(PeterPaul.chem.nutrients, aes(x=factor(month, levels=1:12, labels=month.abb))) + 
  geom_bar()+
  scale_x_discrete(drop=FALSE)+ #to show all the months
  labs(title="My Plot", x="Month")
...
#Create a base plot to play with
surface_temp_vs_dayofyear <- PeterPaul.chem.nutrients %>% 
  filter(depth == 0) %>% 
  ggplot(
    mapping = aes(
      x=daynum,
  ...
  geom_point(size=0.5, alpha=0.5)+ #alpha is transparency 
  facet_wrap(vars(lakename))+ #2plots 2 lakenames
  labs(title="Paul Lake", x="Day of Year", y="Surface Temperature (C)", color="Year")



#Show the basic plot
surface_temp_vs_dayofyear
